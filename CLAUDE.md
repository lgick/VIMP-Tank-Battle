...
- **Клиентский визуальный спавн снарядов (Фаза 5c)**: свой танк предсказывается (5b выполнена), но выстрелы/эффекты по-прежнему появляются только после подтверждения сервером (`delay + RTT`). Нужен немедленный визуальный спавн трассеров/снарядов своего танка; физика/урон остаются серверными.
