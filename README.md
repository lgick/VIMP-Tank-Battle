...
На данном этапе пользователь получает покадровые данные игры из функции `sendShot`,
...
- отправка первого кадра, который включает `keySet` - набор клавиш под статус пользователя (см. пункт 4)
- определяются респауны для игроков на карте
- создаются игроки

4. Отправка первого кадра в начале раунда:
