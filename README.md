...
- создаётся пользователь, но не включается в игровой процесс, не отображён в статистике,
  не участвует в опросах и т.д
- назначается `gameID`, `name`, `teamID` и дефолтные настройки
- статус `user.mapReady` в значении `false`

2. После загрузки карты и подтверждения от пользователя, что карта готова, в функции `mapReady`
   происходит:

- добавление пользователя в модули (`chat`, `vote`, `stat`, `panel`)
- определение ему `lookID` (на данном этапе пользователь не включён в игровой процесс,
  а только наблюдает за игрой)
- отправка полных game-данных
- статус `user.mapReady` в значении `true`

3. На этом этапе пользователь получает покадровые данные игры из функции `createShot`,
   наблюдает за игрой, может переключать игроков. Ему доступны модули `chat`, `vote`, `stat`, `panel`

4. С началом нового раунда запускается функция `startRound`. Она:

- удаляет данные активных игроков `this._playersList = []`
- удаляет всех игроков из модуля `game`
- сбрасывает все динамические элементы карты на исходные позиции в модуле `game`
- сбрасывает все данные пуль в модуле `game`
- заново создаёт карту `this._game.createMap(this._currentMapData)`
- запускает `sendFirstShot` (см. пункт 5)
- определяет для пользователя респаун на карте

5. Функция `sendFirstShot` запускается в начале раунда, она:

- отправляет пустые данные модулей
- отправляет данные в `game`, которые требуется удалить (например данные пуль из прошлого раунда,
  для удаления их с полотна)
- если пользователь решил сменить команду (свойство `nextTeam`), тут происходит смена:
  1. если пользователь стал `spectator`, вносит его в список удалённых игроков
     (чтоб обновить его статус для других в дальнейшем)
  2. если пользователь из наблюдателя стал активным игроком, меняются соответсвующие настройки

6. Функция `createMap` создаёт карту:

- останавливает все таймеры игры
- сбрасывает данные по количеству игроков в командах (`this._allUsersInTeam`)
- сбрасывает данные модуля `stat`
- сбрасывает данные модуля `game`
- создаёт карту в модуле `game`
- корректирует статусы игроков под новые респауны (если требуется)
