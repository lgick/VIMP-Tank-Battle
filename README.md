...
- статус `user.userReady` в значении `false`
...
- определение ему `watchedGameID` (на данном этапе пользователь не включён в игровой процесс,
  а только наблюдает за игрой)
- отправка полных game-данных
- статус `user.userReady` в значении `true`
...
- удаляет данные активных игроков `this._activePlayersList = []`
