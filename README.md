...
Системные сообщения формируются из шаблонных данных из массива ``messageList``

Данные для сообщения являются массивом двух видов:

* [``id (number)``, ``data (array)``] - системное сообщение. Первый параметр - id массива. Второй параметр - массив из дополнительных данных для формирования системного сообщения.
* [``text (string)``, ``name (string)``, ``type (string)``] - сообщение пользователя. Первый параметр - текст сообщения, второй - ник пользователя, третий - тип для класса.
