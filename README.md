...
- параметр `isWatching` в значении `true`, `watchedGameID` имеет `gameID` наблюдаемого игрока
  (на данном этапе пользователь не включён в игровой процесс)
- статус `user.mapReady` в значении `false`
- добавление пользователя в модули (`chat`, `vote`, `stat`, `panel`)

2. После загрузки карты и подтверждения от пользователя, что карта готова (функция `mapReady`)
   происходит:

- статус `user.mapReady` в значении `true`
- если на сервере 2 или меньше игроков и хотя бы один игрок ожидает (`isWatching` === `true`)
  и не является наблюдателем, то раунд начинается заново
- в других случаях отправка полных game-данных всех игроков

На данном этапе пользователь получает покадровые данные игры из функции `createShot`,
наблюдает за игрой, может переключать наблюдаемых игроков.
Ему доступны модули `chat`, `vote`, `stat`, `panel`

3. После запуска нового раунда (функции `this.startRound` и `this.restartRound`) начинается игровой процесс:

- удаляются данные активных игроков `this._activePlayersList = []`
- удаляются все игроки из модуля `game`
- сбрасываются все данные пуль в модуле `game`
- заново создаётся карта функцией `this._game.createMap(this._currentMapData)`
- запускается функция `sendFirstShot` (см. пункт 4)
- определяются респауны для игроков на карте
- создаются игроки

4. Функция `sendFirstShot` запускается в начале раунда, она:
...
5. Функция `createMap` создаёт карту:
...
- также тут отправляются данные об удаленных пользователях, координаты наблюдаемых пользователей для наблюдателей,
  данные `panel` и `keySet` (этот параметр после отправки становится `null`)
