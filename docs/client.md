...
- **подавление серверных дублей** (`filterServerSnapshot`) по id автора в данных события (`tracers[7]`, `bombs[5]`): свои трассеры — FIFO pending-очередь с таймаутом 2 с; своя бомба — remap `serverShotId → localId` (локальный ключ `L<n>` не пересекается с base36-ключами сервера; звук/таймер не перезапускаются, `null` сервера удаляет локальную бомбу).
