  ...
  modules: {
    canvasOptions: {
      vimp: {
        ...
        baseScale: '2.5:1',
        cameraSettings: {
          // плавность движения камеры - определяет,
          // как быстро камера реагирует на изменения скорости и направления.
          // (0.01 - очень плавно/медленно, 0.05 - мгновенно, резко)
          lerpFactor: 0.01,

          // коэффициент смещения камеры вперед (Look-ahead) - как далеко камера
          // заглядывает вперед при движении
          // (5.0 - небольшое смещение, 40.0 - большой обзор впереди)
          lookAheadFactor: 40.0,

          // коэффициент уменьшения зума от скорости - как сильно скорость танка
          // влияет на зум.
          // (0.5 - эффект отдаления заметен при небольшой скорости,
          // 0.01 - эффект отдаления еле заметен)
          zoomOutFactor: 0.5,

          // максимальное отдаление - лимит, чтобы камера не улетела в космос.
          // (в процентах от базового зума, например 0.7 = 70%)
          minZoomLimit: 0.6,
        },
      },
      radar: {
