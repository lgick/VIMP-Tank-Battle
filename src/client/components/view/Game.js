...
export default class GameView {
  constructor(model, app) {
    ...
    this._width = this._app.canvas.width;
    this._height = this._app.canvas.height;
    ...
    this._mPublic.on('updateScreenCoords', 'updateScreenCoords', this);
  ...
  updateScreenCoords(data) {
    const { coords, scale } = data;
    const x = +(this._width / 2 - coords.x * scale).toFixed();
    const y = +(this._height / 2 - coords.y * scale).toFixed();
