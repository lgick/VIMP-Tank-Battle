...
> реализованы (639 тестов зелёные, eslint чист). Следующее: Фаза 5 (prediction/interpolation) —
> только после полной готовности базовой игры. Track B (Rapier) — не начат.
>
...
## Порядок выполнения
~~Фаза 0~~ → ~~1~~ → ~~2~~ → ~~3~~ → ~~4~~ → **5**. Track B — можно начинать,
обязательно до Фазы 5.
