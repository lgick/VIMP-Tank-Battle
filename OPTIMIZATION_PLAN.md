...
> ✅ Фаза 5b (client-side prediction + reconciliation), ✅ Фаза 5c (клиентский спавн снарядов) —
> реализованы (725 тестов зелёные, eslint чист). **Все фазы плана выполнены.**
...
### Фаза 5b — Prediction + reconciliation (свой танк) ✅ ВЫПОЛНЕНО
> Реализовано: `src/client/TankPredictor.js` — реплика серверной модели движения
> (`Tank.updateData` без Rapier-коллизий) фикс-шагом `timeStep` с историей ввода
> (`{time, keysMask, oneShotMask}`, base-маска при подрезке, возраст 2с). Ввод —
> `"seq:action:name"` (`KEYS_DATA`); сервер хранит `HumanParticipant.lastInputSeq`.
> Кадр v2: опциональный per-user **player-блок** (`gameId`, `lastInputSeq`,
> Float32×8 x/y/angle/vx/vy/angvel/gunRotation/throttle **без округления** + флаг центрирования
> башни) — из него клиент узнаёт и свой gameId (отдельный канал не понадобился).
> Reconciliation: состояние := серверное → replay истории от `serverTime` кадра до оценки
> серверного «сейчас» (offset из `SnapshotInterpolator`); расхождение — в `visualError`
> (экспоненциальное затухание, снап при >100 юнитов). Рендер: предсказание перекрывает
> интерполяцию через тот же `CTRL.parse`-конвейер (дискретные поля — из снапшота), камера —
> предсказанная позиция; `SnapshotInterpolator` не менялся (только геттер `offset`).
> Сбросы: `camera[2]`/keySet/смена карты; `condition 0` — заморозка. Параметры реплики —
> `client:prediction` (собирает `src/server/main.js`); константы манеры вождения вынесены из
> Tank в `models.js` (единый источник сервера и реплики).
> **Ключевая находка** (закреплена паритет-тестом `tests/server/TankPredictorParity.test.js` —
> реальный Rapier против реплики, 6 сценариев): Rapier интегрирует позицию/угол скоростью **до**
> демпфирования, хранит задемпфированную (`v *= 1/(1+dt·d)`).
> Отклонения от плана: разделение `LocalTank`/`RemoteTank` не понадобилось (override поверх
> интерполяции тем же конвейером); исключение своего танка из интерполятора не потребовалось.
> Тесты: паритет (6), юниты предиктора (11), кодек v2 (3), lifecycle player-блок (2), VIMP seq.
> 692 теста зелёные, eslint чист.

### Фаза 5c — Снаряды ✅ ВЫПОЛНЕНО
> Реализовано: `src/client/ShotPredictor.js` (чистый класс без Pixi/DOM) — при нажатии fire
> трассер (`w1`) и бомба (`w2`) своего танка спавнятся немедленно через обычный
> `applyGameData`-конвейер; физика/урон/взрыв (`w2e`) — серверные. Решения пользователя:
> объём w1 + w2; конечная точка трассера — клиентский raycast по статике + динамике + танкам.
> - **Гейт** — реплика `tryConsumeAmmoAndShoot`: локальные кулдауны (`fireRate`), патроны из
>   панели (`syncPanel` парсит `'w1:N'`/`'wa:wX'`), активное оружие — локальный цикл на
>   `nextWeapon`/`prevWeapon` + авторитетный `'wa'`; стрельба только живым танком
>   (`condition`) при активном предикте.
> - **Muzzle/direction** — реплика `Tank.getMuzzlePosition`/`getFireDirection` (смещение
>   `width·0.55`, width = `size·4`) от предсказанного состояния `TankPredictor`.
> - **Raycast** — `src/lib/raycast.js`: `rayVsGrid` (DDA по тайлам `physicsStatic`,
>   tileSize = step·scale) + `rayVsBox` (slab-тест: динамика карты `c1`/`c2` и танки
>   `size·4×size·3`, свой танк исключён); позиции целей — `updateWorld` из
>   интерполированных кадров.
> - **Подавление дублей** — кадр v3: сервер помечает события id автора
>   (`w1[7]` shooterId из `HitscanService`, `w2[5]` ownerId из `Bomb.getData`).
>   `filterServerSnapshot` на каждом дискретном кадре: свои трассеры гасятся FIFO
>   pending-очередью (таймаут 2с — отклонённый сервером выстрел не съедает чужие дубли);
>   своя бомба — remap `serverShotId → localId` (`L<n>`, не пересекается с base36):
>   создание подавляется (звук/таймер не перезапускаются), `null` сервера перенаправляется
>   локальной бомбе.
> - Конфиг: `client:prediction` дополнен `weapons` (`src/server/main.js`).
> - Фантомный выстрел при рассинхроне (смерть в полёте ввода) — редкий визуальный артефакт
>   без последствий.
> Тесты: raycast (13), ShotPredictor (17), кодек v3 (+2), lifecycle shooterId/ownerId (+1),
> актуализация HitscanService/Bomb. 725 тестов зелёные, eslint чист.
